...
pub async fn insert_bundle(
    ...
    let query_str = format!(
        "INSERT INTO bundles(optimistic_hash, bundle_txid, data_size, is_settled) VALUES(\"{}\", \"{}\", {}, {})",
        optimistic_hash, bundle_txid, data_size, is_settled as u8
    );
...
pub async fn get_unsettled_bundles() -> Result<Vec<Bundle>, Error> {
    let conn = ps_client().await?;
    let query_str = "SELECT * FROM bundles WHERE is_settled = FALSE";
    let results: Vec<Bundle> = query(&query_str).fetch_all(&conn).await?;
    Ok(results)
}
